        ...
        AppType::Codex | AppType::GrokBuild => {
            ...
            if changed {
                if let Some(obj) = provider.settings_config.as_object_mut() {
                    obj.insert("config".to_string(), Value::String(new_doc.to_string()));
                }
            }
            changed
        }
...
/// Shared takeover check for user-config backfill and live projection.
/// Stale backups alone must not suppress either operation.
pub(crate) fn live_config_owned_by_proxy(state: &AppState, app_type: &AppType) -> bool {
    let has_live_backup =
