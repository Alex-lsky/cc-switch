...
// Internal re-exports
use live::{
    apply_live_user_delta, remove_hermes_provider_from_live, remove_openclaw_provider_from_live,
...
"#
    ...
    async fn update_current_claude_desktop_provider_syncs_profile_when_proxy_takeover_is_active() {
        ...
        // Let the OS allocate a test-only port rather than competing with a running app.
...
"#
    ...
    fn sync_universal_to_apps_reprojects_current_child_to_live() {
        with_test_home(|state, _home| {
            ...
            old_live["env"]["ANTHROPIC_AUTH_TOKEN"] = Value::String("old-key".to_string());
            old_live["permissions"] = json!({"allow": ["Bash(ls:*)"]});
            old_live["spinnerTipsEnabled"] = json!(false);
            ...
            let live: Value = read_json_file(&get_claude_settings_path()).expect("read live");
            assert_eq!(live["permissions"], old_live["permissions"]);
            assert_eq!(live["spinnerTipsEnabled"], json!(false));
            assert_eq!(
                live["env"]["ANTHROPIC_BASE_URL"].as_str(),
                Some("https://api.new.example")
            );
...
impl ProviderService {
    ...
    pub fn sync_current_provider_for_app(
        ...
        Self::sync_current_provider_for_app_impl(state, app_type, true)
    }

    fn sync_current_provider_for_app_impl(
        state: &AppState,
        app_type: AppType,
        backfill_existing_values: bool,
    ) -> Result<(), AppError> {
        if app_type.is_additive_mode() {
        ...
        if !app_type.is_additive_mode() && !live::live_config_owned_by_proxy(state, &app_type) {
            match read_live_settings(app_type.clone()) {
                Ok(live_config) => {
                    if !backfill_existing_values {
                        // An explicit universal-provider edit has already replaced DB values.
                        // Carry only user-owned additions, never resurrect the old URL/key.
                        let snapshot = provider.settings_config.clone();
                        if apply_live_user_delta(&app_type, &live_config, &snapshot, &mut provider)
                        {
                            state.db.save_provider(app_type.as_str(), &provider)?;
                        }
                    } else {
...
impl ProviderService {
    ...
    fn project_universal_child_to_live(
        ...
        if let Err(err) = Self::sync_current_provider_for_app_impl(state, app_type.clone(), false) {
            log::warn!(
