...
- Updated TeamoRouter presets across Claude Code, Claude Desktop, Codex, Grok Build, OpenCode, OpenClaw, Hermes, and Pi to prefer the new `.cn` service domain, while retaining the previous `.com` endpoint as a fallback where endpoint candidates are supported.

### Fixed

- Granted the Tauri process exit capability so recovery screens can reliably close the application after database-version or configuration-load failures.
- Indexed Pi session-usage deduplication lookups by their complete request and semantic identities, improving large-history import performance while preserving duplicate protection.
- Prevented language synchronization rerenders from overwriting unsaved or explicitly reset settings with a stale server-query snapshot.
