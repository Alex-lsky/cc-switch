...
- Integrate upstream f3b18df on the published Pro v3.20.3 baseline, preserving fork identity, Me-zai, config sovereignty, WSL helpers and existing preset policy.
- Include Codex OAuth/config compatibility, image proxy endpoints, parallel tools, session scanning and model/pricing updates. Database schema advances from v17 to v18; back up before upgrading.

### Fixed

- Preserve explicit universal-provider edits instead of backfilling stale live credentials while retaining user-owned additions.
- Resolve owned-Provider borrowing at new upstream call sites; use an ephemeral proxy port and the actual bound URL in takeover tests.
- Verify all updater signatures and required artifacts before staging and publishing a complete release.

See `docs/release-notes/v3.20.4-zh.md` for upgrade and download instructions.

## [Pro 3.20.3] - 2026-08-28

- Published fork release bf550c0 with neutral preset selection, Me-zai, config sovereignty, WSL/platform helpers and build call-site fixes.

## [Pro 3.20.2] - 2026-08-25

- Published release 9058494 with TeamoRouter domain refresh, process-exit capability, Pi dedup indexes and settings language-reset preservation.

## [Pro 3.20.1] - 2026-08-20

### Fixed

- Added a built-in **Reset Codex sign-in state** recovery action that disables Codex takeover, clears managed and native Codex authentication state, preserves providers/MCP/common configuration, and returns the app to the OpenAI Official provider so Windows users can sign in again after a mixed or stuck Meizai login state.


## [Upstream 3.20.2] - 2026-09-07
